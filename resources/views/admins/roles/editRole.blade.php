...
@section('title', 'Editar / Roles | Asistencia UMA')
            ...
            <div class="card-body">
                <form action="{{ route('roles.update', $role->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mx-auto">
                        <div class="col">
                            <x-adminlte-input type="text" class="form-control" id="name" name="name" autocomplete="name" aria-describedby="emailHelp" label="Nombre de Rol" fgroup-class="col-me-3" value="{{ $role->name }}"/>
                        </div>
                    </div>
                                                ...
                                                <div class="col-md-10">
                                                    @foreach ($Permission as $permissions)
                                                        @if (str_starts_with($permissions->name, 'usuario'))
                                                            <label>
                                                                {{ Form::checkbox('permission[]', $permissions->id, in_array($permissions->id, $rolePermissions) ? true : false, array('class' => 'name')) }}{{ $permissions->name }}
                                                            </label><br>
                                                        @endif
                                                    @endforeach
                                                ...
                                                <div class="col-md-10">
                                                    @foreach ($Permission as $permissions)
                                                        @if (str_starts_with($permissions->name, 'rol'))
                                                            <label>{{ Form::checkbox('permission[]', $permissions->id, in_array($permissions->id, $rolePermissions) ? true : false, array('class' => 'name')) }}{{ $permissions->name }}</label><br>
                                                        @endif
                                                    @endforeach
                                                ...
                                                <div class="col-md-10">
                                                    @foreach ($Permission as $permissions)
                                                        @if (str_starts_with($permissions->name, 'catedratico'))
                                                            <label>{{ Form::checkbox('permission[]', $permissions->id, in_array($permissions->id, $rolePermissions) ? true : false, array('class' => 'name')) }}{{ $permissions->name }}</label>
                                                        @endif
                                                    @endforeach
                                                    <br>
                    ...
                    <div class="row mx-auto">
                        ...
                        <div style="margin-left: 10px">
                            <button type="submit" class="btn btn-danger mt-3" id="agregar">Modificar Rol</button>
                        </div>
                    </div>
                </form>
            </div>
